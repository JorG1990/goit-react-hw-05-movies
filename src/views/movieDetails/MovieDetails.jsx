...
import { ContainerDetails, ContainerInformation, ContainerMovieDetails, GoBack } from "./MovieDetails.stiled";
...
const MovieDetails = () => {
  ...
  return (
    <>
      <GoBack>
        <Link to={ cameBack } className="go-back-linck">
        ↶ Go Back
        </Link>
      </GoBack>
      {loading ? (
        ...
        <>
          <ContainerMovieDetails>
            { data.poster_path ? (
              <img alt={ data.original_title } src={ `https://image.tmdb.org/t/p/w500${data.poster_path}` }
              />
            ) : (
              <p>Sorry</p>
            )}
